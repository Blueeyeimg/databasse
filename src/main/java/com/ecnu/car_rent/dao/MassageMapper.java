package com.ecnu.car_rent.dao;

import com.ecnu.car_rent.model.Massage;

import java.util.List;

public interface MassageMapper {

    List<Massage> selectAllMassagesByCarOrderId(int id);//按massage_id正序

    List<Massage> selectAllMassages();//按massage_id正序

    List<Massage> selectAllMassagesBySenderName(String senderName);//按massage_id正序

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table massage
     *
     * @mbggenerated Tue Dec 19 14:39:15 CST 2017
     */
    int deleteByPrimaryKey(Integer massageId);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table massage
     *
     * @mbggenerated Tue Dec 19 14:39:15 CST 2017
     */
    int insert(Massage record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table massage
     *
     * @mbggenerated Tue Dec 19 14:39:15 CST 2017
     */
    int insertSelective(Massage record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table massage
     *
     * @mbggenerated Tue Dec 19 14:39:15 CST 2017
     */
    Massage selectByPrimaryKey(Integer massageId);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table massage
     *
     * @mbggenerated Tue Dec 19 14:39:15 CST 2017
     */
    int updateByPrimaryKeySelective(Massage record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table massage
     *
     * @mbggenerated Tue Dec 19 14:39:15 CST 2017
     */
    int updateByPrimaryKeyWithBLOBs(Massage record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table massage
     *
     * @mbggenerated Tue Dec 19 14:39:15 CST 2017
     */
    int updateByPrimaryKey(Massage record);
}